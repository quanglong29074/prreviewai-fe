...
const App: React.FC = () => {
  ...
  // ✅ Bắt token từ URL nếu có (sau khi GitHub redirect)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const tokenFromUrl = params.get('token');
    const tokenInStorage = localStorage.getItem('jwt');

    const init = async (jwt: string) => {
      try {
  ...
  const renderPage = (): React.ReactNode => {
    switch (currentPage) {
      ...
      case Page.REPOSITORY_SETTINGS:
        return selectedRepo ? (
          <RepositorySettingsPage repository={selectedRepo} onBack={handleBackToRepositories} />
        ) : (
          <RepositoriesPage onNavigateToSettings={handleNavigateToSettings} />
        );
      case Page.PROFILE:
            ...
            <div className="flex items-center justify-between pl-2.5 mb-8">
              <div className="flex items-center">
                <svg
                  ...
                  className="h-8 w-8 mr-3 text-sky-400"
                  viewBox="0 0 20 20"
                  fill="currentColor"
                >
          ...
          <div className="pt-4 mt-4 border-t border-slate-700/50">
            <div className="px-3 mb-4">
              <button
                ...
                className="flex w-full items-center p-2 bg-slate-800/50 rounded-lg hover:bg-slate-700/50 transition-colors"
              >
            ...
            <a
              href="#"
              className="flex items-center p-3 text-sm text-slate-400 hover:text-white hover:bg-slate-700/50 rounded-lg transition-colors group"
            >
              <HelpIcon />
